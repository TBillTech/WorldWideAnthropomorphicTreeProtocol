...
// The redirected backend is a pretty simple concept that makes a sub-tree of the root appear as a complete tree.
// The prefix will be added to all the label rules of this redirected backend to map them to the root backend.

class RedirectedBackend : public Backend {
    public:
        RedirectedBackend(Backend & root, std::string prefix) : root_backend_(root) 
        {
            if (prefix.empty()) {
        ...
        ~RedirectedBackend() override = default;
